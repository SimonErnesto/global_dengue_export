...
for i in tqdm(range(countries)):
    name = df.country.unique()[i]
    name2 = df.abbrev.unique()[i].replace(".", "")
    ...
    for m in range(months):
        ...
        cbar.set_label(label="Risk", size=8)
        ax[u,l].axis("off")
        ax[u,l].set_title(month_names[m], size=12)
    fig.subplots_adjust(top=0.2)
    plt.suptitle(name+" 2020 Posterior Means", size=16)
    plt.subplots_adjust(wspace=2)
    plt.tight_layout()
    plt.savefig("./world_plots/plots_means_"+name2+".png", dpi=300)
    plt.close()
...
for i in tqdm(range(countries)):
    ...
    for m in range(months):
        ...
        ax[u,l].set_title(month_names[m], size=12)
    fig.subplots_adjust(top=0.2)
    plt.suptitle(name+" 2020 Posterior SD", size=16)
    plt.subplots_adjust(wspace=2)
    plt.tight_layout()
    plt.savefig("./world_plots/plots_stds_"+name2+".png", dpi=300)
    plt.close()
